...
namespace P42.Uno.DataTransfer.Droid
{
    public class ClipboardService : P42.Uno.DataTransfer.INativeClipboardService
    {
        ...
        public ClipboardService()
        {
            Clipboard.PrimaryClipChanged += (sender, e) =>
            {
                ...
                P42.Uno.DataTransfer.Clipboard.OnContentChanged(this, EventArgs.Empty);
            };
        }
        ...
        // (via new P42.Uno.DataTransfer.Droid.ClipboardEntry())
        // before Clipboard.PrimaryClipChanged is fired so that we don't whipe 
        // _lastEntry and instantiate another P42.Uno.DataTransfer.Droid.ClipboardEntry.
        ...
        */
        ...
        #region Entry
        ClipData _lastClipData;
        IMimeItemCollection _lastEntry;
        public IMimeItemCollection Entry
        {
            get
            {
                return _lastEntry = _lastEntry ?? new P42.Uno.DataTransfer.Droid.MimeItemCollection();
            }
            set
            {
                try
                {
                    if (_lastClipData != null)
                    {
                        for (int i = _lastClipData.ItemCount - 1; i >= 0; i--)
                            _lastClipData.GetItemAt(i)?.Dispose();
                    }
                    _lastClipData?.Dispose();
                    _lastClipData = null;
                    var items = value.AsClipDataItems();
                    if (items.Count > 0)
                    {
                        _lastClipData = new ClipData(value.Description, value.MimeTypes().ToArray(), items[0]);
                        if (items.Count > 1)
                        {
                            for (int i = 1; i < items.Count; i++)
                                _lastClipData.AddItem(items[i]);
                        }
                    }
                    _lastEntry = EntryCaching ? value : null;
                    _locallyUpdated = true;
                    Clipboard.PrimaryClip = _lastClipData ?? ClipData.NewPlainText("", "");
                }
                catch (Exception e)
                {
    ...
    [ContentProvider(new string[] { "${applicationId}.p42.uno.clipboardcontentprovider.authority" }, Enabled = true, Exported = true, GrantUriPermissions = true)]
    public class ClipboardContentProvider : ContentProvider
    {
        ...
        static P42.Uno.DataTransfer.IMimeItem ItemForUri(Android.Net.Uri uri)
        {
        ...
        public static string AUTHORITY => Android.App.Application.Context.PackageName + ".p42.uno.clipboardcontentprovider.authority";
        ...
        {
            //_resourceId = GetResourceId("P42.Uno.DataTransfer_copy_paste_authority", "string", Settings.Activity.PackageName);
        ...
        {
            System.Diagnostics.Debug.WriteLine(nameof(Shutdown));
            base.Shutdown();
        }

        public override AssetFileDescriptor OpenTypedAssetFile(Android.Net.Uri uri, string mimeTypeFilter, Bundle opts)
        {
            System.Diagnostics.Debug.WriteLine(nameof(OpenTypedAssetFile) +" A");
        ...
        {
            System.Diagnostics.Debug.WriteLine(nameof(OpenAssetFile) + " A");
        ...
        public override ParcelFileDescriptor OpenFile(Android.Net.Uri uri, string mode)
        {
            ...
            System.Diagnostics.Debug.WriteLine(nameof(OpenFile) + " A");
        ...
        {
            System.Diagnostics.Debug.WriteLine(nameof(OpenFile) + " B");
